...
def openai_call(
    ...
    if engine == "mock" or os.environ.get("USE_MOCK_LLM") == "1" or not os.environ.get("OPENAI_API_KEY"):
        from commaqa.models.mock_generator import MockGenerator
        mg = MockGenerator(engine=engine)
        seqs = mg.generate_text_sequence(prompt)
        class MockChoice(dict):
            def __init__(self, text):
                super().__init__({"text": text, "logprobs": None})
        class MockResponse(dict):
            def __init__(self, text):
                super().__init__({"choices": [MockChoice(text)]})
        return MockResponse(seqs[0][0])
    ...
    try:
        ...
        print(f"[GPT3Generator] OpenAI API call failed ({e}), falling back to MockGenerator.")
        from commaqa.models.mock_generator import MockGenerator
        mg = MockGenerator(engine=engine)
        seqs = mg.generate_text_sequence(prompt)
        class MockChoice(dict):
