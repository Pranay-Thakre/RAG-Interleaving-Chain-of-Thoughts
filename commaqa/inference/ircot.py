...
_inprocess_retriever = None

def safe_post_request(url, params):
    global _inprocess_retriever
    try:
        res = requests.post(url, json=params, timeout=2)
        if res.ok:
            return res
    except Exception:
        pass
    
    # Fallback to direct in-process retrieval
    if _inprocess_retriever is None:
        import sys
        if "retriever_server" not in sys.path:
            sys.path.append("retriever_server")
        from unified_retriever import UnifiedRetriever
        _inprocess_retriever = UnifiedRetriever()
        
    retrieval_method = params.get("retrieval_method", "retrieve_from_elasticsearch")
    args_copy = dict(params)
    if "retrieval_method" in args_copy:
        del args_copy["retrieval_method"]
    retrieval = getattr(_inprocess_retriever, retrieval_method)(**args_copy)
    ...
    class MockHTTPResponse:
        def __init__(self, data):
            ...
            self._data = data
        def json(self):
            return {"retrieval": self._data, "time_in_seconds": 0.01}
            
    return MockHTTPResponse(retrieval)
            ...
            for allowed_paragraph_type in self.allowed_paragraph_types:
                ...
                if result.ok:
                    ...
                    for retrieval_item in retrieval:
                        ...
                        if self.valid_titles_are_allowed_titles and state.data.get("valid_titles"):
                            if retrieval_item["title"].lower().replace(" ", "") not in [
